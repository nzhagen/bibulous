...
**.if_singular(var1,var2,var3)** inserts ``var2`` if ``var1`` has only one element, but ``var3`` if ``var1`` has more than one element. Here ``var1`` is assumed to be a list-type of variable, and ``var2`` and ``var3`` are assumed to be either fields present within the database entry or variables defined in the ``SPECIAL-TEMPLATES`` section of the file.

**.if_length_equals(var1,number,var2,var3)** inserts ``var2`` if ``var1`` has ``number`` elements, but otherwise inserts ``var3``. Here ``var1`` is assumed to be a list-type of variable, and ``var2`` and ``var3`` are assumed to be string-type variables (i.e. fields present within the database entry or variables defined in the ``SPECIAL-TEMPLATES`` section of the file).

**.if_length_less_than(var1,number,var2,var3)** inserts ``var2`` if ``var1`` has less than ``number`` elements, but otherwise inserts ``var3``. Here ``var1`` is assumed to be a list-type of variable, and ``var2`` and ``var3`` are assumed to be string-type variables (i.e. fields present within the database entry or variables defined in the ``SPECIAL-TEMPLATES`` section of the file).

**.if_length_more_than(var1,number,var2,var3)** inserts ``var2`` if ``var1`` has more than ``number`` elements, but otherwise inserts ``var3``. Here ``var1`` is assumed to be a list-type of variable, and ``var2`` and ``var3`` are assumed to be string-type variables (i.e. fields present within the database entry or variables defined in the ``SPECIAL-TEMPLATES`` section of the file).
